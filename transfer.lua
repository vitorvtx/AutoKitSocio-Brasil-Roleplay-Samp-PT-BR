--[[
    ...
    Criador: vitorvtx
    Comando: /transfer
    v4.1 - CAIXA (F+ID) e PORTAMALAS (N)
]]
...
local function nome_tipo(t)
    if t == 1 then return "PORTAMALAS" end
    if t == 2 then return "CAIXA" end
    return "?"
end
...
local function organizar()
    ...
    for pass = 1, C.org_max do
        ...
        wait(100)
        if not guardar_item(S.id_org, 2, item) then set_status("Falha ao guardar"); break end
...
local function transferir()
    ...
    local o = nome_tipo(S.orig_tipo) .. (S.orig_tipo==2 and (" "..tostring(S.id_origem)) or "")
    local d = nome_tipo(S.dest_tipo) .. (S.dest_tipo==2 and (" "..tostring(S.id_destino)) or "")
...
imgui.OnFrame(function() return S.menu end, function()
    ...
    imgui.Text("ORIGEM")
    if btn_sel("PORTAMALAS", S.orig_tipo==1, "o1") then S.orig_tipo=1 end
    imgui.SameLine()
    if btn_sel("CAIXA", S.orig_tipo==2, "o2") then S.orig_tipo=2 end
    ...
    imgui.Spacing()
    imgui.Text("DESTINO")
    if btn_sel("PORTAMALAS", S.dest_tipo==1, "d1") then S.dest_tipo=1 end
    imgui.SameLine()
    if btn_sel("CAIXA", S.dest_tipo==2, "d2") then S.dest_tipo=2 end
    ...
    imgui.TextColored(imgui.ImVec4(1.0, 0.75, 0.35, 1), "ORGANIZAR CAIXA")
    ...
    imgui.Spacing()
    imgui.TextColored(imgui.ImVec4(0.40, 0.50, 0.70, 0.65), "PM=N | CAIXA=F+ID | /transfer")
...
function main()
    ...
    sampAddChatMessage("{00BFFF}[Transfer] {FFFFFF}Carregado v4.1 | CAIXA + PORTAMALAS", -1)
