...
local _last_local_err = 0
local function norm_msg(t)
    if type(t) ~= "string" then return "" end
    t = t:gsub("{%x%x%x%x%x%x}", ""):gsub("~%w~", ""):gsub("%s+", " "):upper()
    local map = {["Á"]="A",["À"]="A",["Ã"]="A",["Â"]="A",["É"]="E",["Ê"]="E",["Í"]="I",["Ó"]="O",["Õ"]="O",["Ô"]="O",["Ú"]="U",["Ç"]="C",
        ["á"]="A",["à"]="A",["ã"]="A",["â"]="A",["é"]="E",["ê"]="E",["í"]="I",["ó"]="O",["õ"]="O",["ô"]="O",["ú"]="U",["ç"]="C"}
    for a, b in pairs(map) do t = t:gsub(a, b) end
    return t
end

local function desligar_local_errado()
    local t = now()
    if t - _last_local_err < 3000 then return end
    _last_local_err = t
    if not (S.caixa or S.comer or S.dormir) then return end
    S.caixa, S.comer, S.dormir = false, false, false
    S.busy, S.busy_t, S.em_st = false, 0, false
    S.next_kit, S.next_e, S.next_st = 0, 0, 0
    S.fill_f, S.fill_s, S.fill_o = false, false, false
    pcall(soltar)
    sampAddChatMessage("{FF0000}[AutoKit] {FFFFFF}Voce precisa estar no local adequado para realizar essa funcao.", -1)
    sampAddChatMessage("{FFAA00}[AutoKit] {FFFFFF}Funcoes desativadas automaticamente.", -1)
end

local function checar_msg_local(text)
    if type(text) ~= "string" or text == "" then return end
    local t = text:lower():gsub("{%x%x%x%x%x%x}", ""):gsub("~%w~", "")
    -- Erro: Voce nao esta em sua casa ou na base de sua organizacao.
    if t:find("casa", 1, true) and t:find("organiza", 1, true) then
        desligar_local_errado()
    end
end


function sampev.onServerMessage(_, text)
    checar_msg_local(text)
end
function sampev.onChatMessage(_, text)
    checar_msg_local(text)
end
...
function main()
    ...
    sampAddChatMessage("{00BFFF}[Auto KitSocio] {FFFFFF}v9.7 carregado! INSERT / /autokit", -1)
